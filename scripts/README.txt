...
1. 先使用`./update-rpis.py > online-hosts` 生成树莓派机器的IP列表

2. 树莓派扩容
树莓派的SD卡默认并没有将全部空间占满，所以需要expand fs一下

```
ansible-playbook -i online-hosts expand-pi-fs.yml
```
...
```
ansible-playbook -i gray-hosts playbooks/update-u2init.yml
```
